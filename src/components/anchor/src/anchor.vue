...
//        container: [String, HTMLElement],  // HTMLElement 在 SSR 下不支持
    ...
    computed: {
        wrapperComponent () {
            return this.affix ? 'moc-affix' : 'div';
        },
        wrapperStyle () {
            return {
    ...
    methods: {
        ...
        updateTitleOffset () {
            const links = findComponentsDownward(this, 'mocAnchorLink').map(link => {
                return link.href;
            });
        ...
        init () {
            // const mocAnchorLink = findComponentDownward(this, 'mocAnchorLink');
